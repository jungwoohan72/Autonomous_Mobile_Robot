...
void motor_control() {
		...
		if(bump==1) check_vel(10.0, 8.0);
		if(bump==2 && slope==0){
			if(bump_wait<4){
				// if(lower_k==2){
				// 	left_wheel = right_wheel = 10;
				// 	cout<<"bump_wait is "<<bump_wait<<endl;
				// }
				// else left_wheel = right_wheel = 0;
				left_wheel = right_wheel = 0;
			}
			else if(num < 34 && bump_wait<15){
				left_wheel = 4;
				right_wheel = -4;
				slope_entrance = 0;
			}
				...
				check_vel(6.0,-2.0);
				if(num<33 && slope_entrance>1 && lower_num>10){
					left_wheel = 4;
					right_wheel = -6;
					Ki = 0;
					cout<<"slope_entrance"<<endl;
				}
			}
		}

		if(bump==2 && slope==2){
			if(bump_wait<4) left_wheel = right_wheel = 10;
			else if(num < 34 && bump_wait<15){
				left_wheel = -4;
				right_wheel = 4;
			}
			else check_vel(6.0,-2.0);
		}
...
void lower_image_detect()
{
  ...
  for (int i = 0; i<1080; i++){
	...
	lower_k = numberofblacks.size();

  //cout <<"lower_num is " << lower_num << "       lower_k is " << lower_k << "\n";
}
