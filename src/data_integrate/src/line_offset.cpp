...
void image_detect()
{
  ...
  //**drawing on the image to visualize the offsets

  image_1 = calibrated_frame.clone();
  line(image_1, Point(960,1), Point(960,1079), Scalar(255,0,0),5,8,0);
  for (int t=0; t<num; t+=2)
  {
      circle(image_1, Point(960-average_distances[t+1],average_distances[t]),10, Scalar(0,0,255), CV_FILLED,8,0);
      line(image_1, Point(960,average_distances[t]), Point(960-average_distances[t+1],average_distances[t]), Scalar(0,0,255),5,8,0);
      string text;
      string of = floatToString(average_distances[t+1]);
      text = "  Offset: " + of;
      putText(image_1, text, Point(960-average_distances[t+1],average_distances[t]-5), 2, 1, Scalar(0,255,0), 2);
  }
  imshow("Frame3",image_1);
}
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "image_offset");
  ros::NodeHandle n;
